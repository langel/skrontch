...
static int display_index_valid(int display_index)
{
	int count = SDL_GetNumVideoDisplays();
	return display_index >= 0 && display_index < count;
}

static int window_offscreen_on_display(int display_index, int x, int y, int w, int h)
{
	SDL_Rect display = { 0, 0, 0, 0 };
	if (SDL_GetDisplayBounds(display_index, &display) != 0) {
		return 0;
	}
	...
	int *active_tab, int *x, int *y, int *w, int *h, int *use_position)
{
    ...
    while (1) {
        ...
        if (strcmp(key, "x") == 0) {
            ...
            active = (int)number;
		} else if (strcmp(key, "display_index") == 0) {
			if (!json_parse_number(cursor, &number)) {
				return 0;
			}
	...
	if (found_position && display_index >= 0) {
		if (!display_index_valid(display_index)) {
			found_position = 0;
		} else if (window_offscreen_on_display(display_index, pos_x, pos_y, width, height)) {
			found_position = 0;
		}
	}
...
static int workspace_save_file(const char *path, const app_state_t *app)
{
    ...
    for (int i = 0; i < app->window_count; ++i) {
        ...
        fprintf(file, "      \"w\": %d,\n", w);
        fprintf(file, "      \"h\": %d,\n", h);
		int display_index = SDL_GetWindowDisplayIndex(app->windows[i].window);
		fprintf(file, "      \"active_tab\": %d,\n", app->windows[i].active_tab);
		fprintf(file, "      \"display_index\": %d,\n", display_index);
        fprintf(file, "      \"tabs\": [\n");
