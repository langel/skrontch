...
static void pane_counter_seed_from_tab(const tab_state_t *tab)
{
    ...
    for (int i = 0; i < tab->node_count; ++i) {
        ...
        if (tab->nodes[i].pane_id > pane_counter) {
            pane_counter = tab->nodes[i].pane_id;
        }
    }
}

static void pane_counter_seed_from_window(const window_state_t *window)
{
    if (window == NULL) {
        return;
    }
...
void window_manager_sync_mouse_inside(window_state_t *window)
{
	...
	int inside = global_x >= win_x && global_y >= win_y &&
		global_x < win_x + win_w && global_y < win_y + win_h;
	cursor_manager_set_mouse_inside(&window->cursor_manager, inside);
}
...
enum {
	...
	RESIZE_MODE_RIGHT = 2,
	RESIZE_MODE_TOP = 3,
	RESIZE_MODE_BOTTOM = 4,
	RESIZE_MODE_TOPLEFT = 5,
	RESIZE_MODE_TOPRIGHT = 6,
	RESIZE_MODE_BOTTOMLEFT = 7,
	RESIZE_MODE_BOTTOMRIGHT = 8
};

static int window_get_resize_mode(window_state_t *window, int x, int y)
{
	if (window == NULL) {
	...
	int left = x < WINDOW_RESIZE_MARGIN;
	int right = x >= w - WINDOW_RESIZE_MARGIN;
	int top = y < WINDOW_RESIZE_MARGIN;
	int bottom = y >= h - WINDOW_RESIZE_MARGIN;

	if (top && left) {
		return RESIZE_MODE_TOPLEFT;
	}
	if (top && right) {
		return RESIZE_MODE_TOPRIGHT;
	}
	if (bottom && left) {
		return RESIZE_MODE_BOTTOMLEFT;
	}
	if (bottom && right) {
		return RESIZE_MODE_BOTTOMRIGHT;
	}
	if (top) {
		return RESIZE_MODE_TOP;
	}
	if (bottom) {
		return RESIZE_MODE_BOTTOM;
	}
	if (left) {
		return RESIZE_MODE_LEFT;
	}
	if (right) {
		return RESIZE_MODE_RIGHT;
	}
	return RESIZE_MODE_NONE;
}
...
static SDL_Rect get_menu_bar_rect(window_state_t *window)
{
	...
	return make_rect(WINDOW_BORDER_PADDING, WINDOW_BORDER_PADDING,
		width - WINDOW_BORDER_PADDING * 2, MENU_HEIGHT);
}

static int build_window_asset_path(char *buffer, size_t buffer_size, const char *filename)
{
...
{
	int width = window != NULL ? window->width : 0;
	int y = WINDOW_BORDER_PADDING + MENU_HEIGHT + MENU_BORDER_THICKNESS;
	return make_rect(WINDOW_BORDER_PADDING, y,
		width - WINDOW_BORDER_PADDING * 2, MENU_HEIGHT);
}
...
static SDL_Rect get_menu_max_rect(window_state_t *window)
{
	int width = window != NULL ? window->width : 0;
...
static SDL_Rect get_menu_min_rect(window_state_t *window)
{
	int width = window != NULL ? window->width : 0;
	int x = width - MENU_BUTTON_PADDING * 3 - MENU_BUTTON_SIZE * 3;
	int y = (MENU_HEIGHT - MENU_BUTTON_SIZE) / 2;
	return make_rect(x, y, MENU_BUTTON_SIZE, MENU_BUTTON_SIZE);
}
...
static SDL_Rect get_tab_strip_rect(window_state_t *window)
{
	SDL_Rect bar = get_tab_bar_rect(window);
	int right_reserved = MENU_BUTTON_PADDING * 2 + MENU_BUTTON_SIZE * 2;
	int x = bar.x + TAB_PADDING;
	int w = bar.w - right_reserved - TAB_PADDING * 2;
	if (w < 0) {
		w = 0;
	}
	return make_rect(x, bar.y, w, bar.h);
}
...
static void draw_minus_icon(SDL_Renderer *renderer, const SDL_Rect *rect, int thickness)
{
	if (renderer == NULL || rect == NULL) {
		return;
	}
	...
	int mid_y = rect->y + rect->h / 2;
	int start_x = rect->x + 3;
	int end_x = rect->x + rect->w - 4;
	for (int i = 0; i < thickness; ++i) {
		SDL_RenderDrawLine(renderer, start_x, mid_y + i, end_x, mid_y + i);
	}
}

static void draw_box_icon(SDL_Renderer *renderer, const SDL_Rect *rect, int thickness)
{
	if (renderer == NULL || rect == NULL) {
		return;
	}
    ...
    int *leaf_count, split_bar_t *bars, int *bar_count)
{
    ...
    *leaf_count = 0;
    *bar_count = 0;
	int content_height = window->height - WINDOW_BORDER_PADDING * 2 -
		MENU_HEIGHT - MENU_BORDER_THICKNESS -
		MENU_HEIGHT - MENU_BORDER_THICKNESS;
    if (content_height < 0) {
        content_height = 0;
    }
	SDL_Rect root_rect = make_rect(WINDOW_BORDER_PADDING,
		WINDOW_BORDER_PADDING + MENU_HEIGHT + MENU_BORDER_THICKNESS +
			MENU_HEIGHT + MENU_BORDER_THICKNESS,
		window->width - WINDOW_BORDER_PADDING * 2, content_height);
    split_tree_layout_node(tab, tab->root_node, root_rect, 0,
        leaves, leaf_count, bars, bar_count);
    return 1;
}

static void tab_state_init_default_with_ids(tab_state_t *tab)
{
    if (tab == NULL) {
        return;
    }
    ...
    int left_top = split_tree_add_leaf(tab, ++pane_counter);
    int left_bottom = split_tree_add_leaf(tab, ++pane_counter);
    int right_top = split_tree_add_leaf(tab, ++pane_counter);
    int right_bottom = split_tree_add_leaf(tab, ++pane_counter);
    ...
    SDL_Rect *add_rect)
{
	...
	SDL_Rect bar = get_tab_strip_rect(window);
    int count = 0;
    int x = bar.x;
	int y = bar.y + (bar.h - TAB_HEIGHT) / 2;
    ...
    if (add_rect != NULL) {
        int add_x = x;
		int add_y = bar.y + (bar.h - TAB_ADD_SIZE) / 2;
...
static int window_manager_handle_mouse_down(window_state_t *window, int mouse_x, int mouse_y)
{
	...
	int resize_mode = window_get_resize_mode(window, mouse_x, mouse_y);
	if (resize_mode != RESIZE_MODE_NONE) {
		window->is_resizing_window = 1;
		window->resize_mode = resize_mode;
		SDL_GetWindowPosition(window->window, &window->resize_start_x, &window->resize_start_y);
		SDL_GetWindowSize(window->window, &window->resize_start_w, &window->resize_start_h);
		SDL_GetGlobalMouseState(&window->resize_mouse_start_x, &window->resize_mouse_start_y);
	...
	SDL_Rect menu_max = get_menu_max_rect(window);
	SDL_Rect menu_min = get_menu_min_rect(window);
	SDL_Rect tab_rect = get_tab_bar_rect(window);
	...
	if (point_in_rect(mouse_x, mouse_y, &menu_rect)) {
		...
		window->is_dragging_menu = 1;
		window->hover_menu_bar = 1;
		int win_x = 0;
		int win_y = 0;
		SDL_GetWindowPosition(window->window, &win_x, &win_y);
		int global_x = 0;
		int global_y = 0;
		SDL_GetGlobalMouseState(&global_x, &global_y);
		window->window_drag_offset_x = global_x - win_x;
		window->window_drag_offset_y = global_y - win_y;
		window_manager_update_cursor(window);
		return 0;
	}

	if (point_in_rect(mouse_x, mouse_y, &tab_rect)) {
		...
		if (point_in_rect(mouse_x, mouse_y, &tab_add)) {
			if (window->tab_count < MAX_TABS) {
				tab_state_init_default_with_ids(&window->tabs[window->tab_count]);
        ...
        if (header_rect.w >= HEADER_BUTTON_SIZE + HEADER_BUTTON_PADDING * 2 &&
            ...
            if (leaf_count > 1) {
				...
				if (window->tab_count > 1) {
					int removed_index = window->active_tab;
					for (int i = removed_index; i < window->tab_count - 1; ++i) {
						window->tabs[i] = window->tabs[i + 1];
					}
					window->tab_count -= 1;
					int next_index = removed_index - 1;
					if (next_index < 0) {
						next_index = window->tab_count - 1;
					}
					window->active_tab = next_index;
					window_manager_update_cursor(window);
					return 1;
				}
				tab->node_count = 0;
				...
				tab->focused_pane_node = -1;
				window->should_close = 1;
				if (window->app != NULL && window->app->window_count == 1) {
...
static void window_manager_update_cursor(window_state_t *window)
{
	...
	} else if (window->is_resizing_window || window->resize_mode != RESIZE_MODE_NONE) {
		int mode = window->resize_mode;
		if (mode == RESIZE_MODE_TOPLEFT || mode == RESIZE_MODE_BOTTOMRIGHT) {
			kind = CURSOR_KIND_RESIZE_NWSE;
		} else if (mode == RESIZE_MODE_TOPRIGHT || mode == RESIZE_MODE_BOTTOMLEFT) {
			kind = CURSOR_KIND_RESIZE_NESW;
		} else if (mode == RESIZE_MODE_LEFT || mode == RESIZE_MODE_RIGHT) {
			kind = CURSOR_KIND_RESIZE_WE;
		} else {
			kind = CURSOR_KIND_RESIZE_NS;
		}
	...
	} else if (window->is_dragging_menu) {
		kind = CURSOR_KIND_HAND_CLOSED;
	...
	int use_os_cursor = window->is_resizing_window || window->resize_mode != RESIZE_MODE_NONE;
...
skrontch_error_t window_manager_init(window_state_t *window, const char *title, int width, int height,
    int x, int y, int use_position)
 {
    ...
    tab_state_init_default_with_ids(&window->tabs[0]);
    window->tab_count = 1;
    window->active_tab = 0;
	...
	if (build_window_asset_path(logo_path, sizeof(logo_path), "skrontch_header.png")) {
		logo_surface = image_loader_load_rgba(logo_path);
	}
	if (logo_surface != NULL) {
		window->menu_logo_width = logo_surface->w;
		window->menu_logo_height = logo_surface->h;
		window->menu_logo_texture = SDL_CreateTextureFromSurface(window->renderer, logo_surface);
		if (window->menu_logo_texture != NULL) {
			SDL_SetTextureScaleMode(window->menu_logo_texture, SDL_ScaleModeNearest);
		} else {
			window->menu_logo_width = 0;
			window->menu_logo_height = 0;
		}
...
void window_manager_shutdown(window_state_t *window)
 {
	...
	if (window->menu_logo_texture != NULL) {
		SDL_DestroyTexture(window->menu_logo_texture);
		window->menu_logo_texture = NULL;
	}
	window->menu_logo_width = 0;
	window->menu_logo_height = 0;
...
int window_manager_handle_event(window_state_t *window, const SDL_Event *event)
{
    ...
    if (event->type == SDL_WINDOWEVENT) {
		...
		window->hover_menu_bar = 0;
            ...
            window->hover_tab_add = 0;
            window->hover_header_detach = 0;
            window->resize_mode = RESIZE_MODE_NONE;
            cursor_manager_set_mouse_inside(&window->cursor_manager, 0);
        }
    ...
    if (event->type == SDL_MOUSEMOTION) {
		...
		if (point_in_rect(mouse_x, mouse_y, &menu_rect) &&
			(point_in_rect(mouse_x, mouse_y, &menu_close) ||
			 point_in_rect(mouse_x, mouse_y, &menu_min) ||
			 point_in_rect(mouse_x, mouse_y, &menu_max))) {
			window->hover_menu_button = 1;
		} else if (point_in_rect(mouse_x, mouse_y, &menu_rect)) {
			window->hover_menu_bar = 1;
		}
		...
		if (window->is_dragging_menu) {
			int global_x = 0;
			int global_y = 0;
			SDL_GetGlobalMouseState(&global_x, &global_y);
			int new_x = global_x - window->window_drag_offset_x;
			int new_y = global_y - window->window_drag_offset_y;
	...
	if (event->type == SDL_KEYDOWN && tab != NULL) {
		if (event->key.keysym.sym == SDLK_LEFT &&
			...
			if (window->tab_count > 0) {
				int next_index = window->active_tab - 1;
				if (next_index < 0) {
					next_index = window->tab_count - 1;
				}
				window->active_tab = next_index;
		...
		if (event->key.keysym.sym == SDLK_RIGHT &&
			...
			if (window->tab_count > 0) {
				int next_index = window->active_tab + 1;
				if (next_index >= window->tab_count) {
					next_index = 0;
				}
				window->active_tab = next_index;
				state_changed = 1;
...
void window_manager_render(window_state_t *window)
 {
	...
	if (window->menu_logo_texture != NULL && window->menu_logo_width > 0 &&
		window->menu_logo_height > 0) {
		SDL_Rect logo_rect = {
			menu_rect.x,
			menu_rect.y,
			window->menu_logo_width,
			window->menu_logo_height
		};
		SDL_RenderCopy(window->renderer, window->menu_logo_texture, NULL, &logo_rect);
	}

	SDL_Rect tab_bar_rect = get_tab_bar_rect(window);
	SDL_SetRenderDrawColor(window->renderer, menu_color.r, menu_color.g, menu_color.b, menu_color.a);
	SDL_RenderFillRect(window->renderer, &tab_bar_rect);
	...
	if (window->is_dragging_tab && window->hover_tab_insert >= 0) {
		...
		SDL_SetRenderDrawColor(window->renderer, tab_icon.r, tab_icon.g, tab_icon.b, tab_icon.a);
		SDL_RenderDrawLine(window->renderer, line_x, tab_bar_rect.y + 4,
			line_x, tab_bar_rect.y + tab_bar_rect.h - 4);
	}

	SDL_Color border_color = palette_get_color("black");
	SDL_SetRenderDrawColor(window->renderer, border_color.r, border_color.g, border_color.b, border_color.a);
	SDL_Rect menu_border = make_rect(WINDOW_BORDER_PADDING,
		WINDOW_BORDER_PADDING + MENU_HEIGHT,
		window->width - WINDOW_BORDER_PADDING * 2, MENU_BORDER_THICKNESS);
	SDL_RenderFillRect(window->renderer, &menu_border);
	SDL_Rect tab_border = make_rect(WINDOW_BORDER_PADDING,
		WINDOW_BORDER_PADDING + MENU_HEIGHT + MENU_BORDER_THICKNESS + MENU_HEIGHT,
		window->width - WINDOW_BORDER_PADDING * 2, MENU_BORDER_THICKNESS);
	SDL_RenderFillRect(window->renderer, &tab_border);
	...
	SDL_Rect menu_max = get_menu_max_rect(window);
	SDL_Rect menu_close = get_menu_close_rect(window);
	draw_minus_icon(window->renderer, &menu_min, 2);
	draw_box_icon(window->renderer, &menu_max, 2);
	draw_x_icon(window->renderer, &menu_close, 2);

	SDL_Color window_border = palette_get_color("gray1");
	SDL_SetRenderDrawColor(window->renderer, window_border.r, window_border.g,
		window_border.b, window_border.a);
